...
for i in ${HEADER_INTERNAL_FILES}; do
	SRC_URI+=" headers-only? ( https://developer.download.nvidia.com/redist/optix/v7.6/internal/${i} -> ${P}-internal-${i} )"

done
for i in ${HEADER_FILES}; do
	SRC_URI+=" headers-only? ( https://developer.download.nvidia.com/redist/optix/v7.6/${i} -> ${P}-${i} )"
done
unset i
