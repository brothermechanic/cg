...
src_install() {
	...
	newicon "${S}"/meshlab/images/eye512.png "${PN}".png || die
	make_desktop_entry meshlab "Meshlab" "${PN}" Graphics || die
	
}

pkg_postinst() {
	elog
	elog "If you are running on KDE"
	elog "Please, add"
	elog "[Development]"
	elog "AutoCheckAccelerators=false"
	elog "to ~/.config/kdeglobals"
	elog
}
