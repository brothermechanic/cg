...
src_compile() {
	...
	if use doc; then
		...
		"${BUILD_DIR}"/bin/blender3 --background --python doc/python_api/sphinx_doc_gen.py -noaudio || die "blender failed."
...
src_install() {
	...
	pax-mark m "${BUILD_DIR}"/bin/blender3
    ...
    rm ${ED%/}/usr/share/icons/hicolor/symbolic/apps/blender-symbolic.svg
    rm ${ED%/}/usr/share/icons/hicolor/scalable/apps/blender.svg
	mv "${ED%/}/usr/bin/blender" "${ED%/}/usr/bin/blender3" || die
	mv "${ED%/}/usr/share/applications/blender.desktop" "${ED%/}/usr/share/applications/blender3.desktop" || die
	sed -i -e "s|^Exec.*|Exec=blender3 %f|" "${ED%/}/usr/share/applications/blender3.desktop"
	sed -i -e "s|^Name.*|Name=Blender3 %f|" "${ED%/}/usr/share/applications/blender3.desktop"
}
