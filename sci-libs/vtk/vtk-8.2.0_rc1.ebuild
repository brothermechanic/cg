...
#	https://github.com/Kitware/VTK/archive/${PV} -> ${P}.tar.gz
SRC_URI="
	https://www.vtk.org/files/release/${SPV}/VTK-${PV/_/.}.tar.gz
	doc? ( https://www.vtk.org/files/release/${SPV}/vtkDocHtml-${PV/_/.}.tar.gz )
	examples? (
		https://www.vtk.org/files/release/${SPV}/VTKData-${PV/_/.}.tar.gz
		https://www.vtk.org/files/release/${SPV}/VTKLargeData-${PV/_/.}.tar.gz
	)"
...
IUSE="
	...
	video_cards_nvidia views web R +X xdmf"

REQUIRED_USE="
	all-modules? ( python xdmf boost )
	java? ( qt5 )
...
PATCHES=(
	"${FILESDIR}/${PN}-8.1.2-fix-designer-plugin-install-dir.patch"
	"${FILESDIR}/${PN}-8.2.0-QVTKOpenGLNativeWidget_cxx.patch"
)
...
src_prepare() {
	local x
	# missing: VPIC freerange utf8 verdict
	# diy2 kissfft pugixml exodusII xdmf2 xdmf3
	for x in doubleconversion eigen expat gl2ps glew freetype hdf5 jpeg jsoncpp libharu libproj libxml2 lz4 lzma mpi4py netcdf ogg pegtl png sqlite theora tiff zlib; do
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DVTK_USE_SYSTEM_PEGTL=ON
		-DVTK_USE_SYSTEM_PUGIXML=ON
		-DVTK_USE_SYSTEM_EXODUSII=ON
	...
	# IO
		...
		-DModule_vtkIOXdmf2=$(usex xdmf)
		-DModule_vtkIOXdmf3=$(usex xdmf)
