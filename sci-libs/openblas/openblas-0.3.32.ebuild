...
src_install() {
	...
	if use eselect-ldso; then
		...
		use fortran && doexe interface/libblas.so.3
		use fortran && dosym libblas.so.3 /usr/${libdir}/blas/openblas/libblas.so
		...
		use fortran && doexe interface/liblapack.so.3
		use fortran && dosym liblapack.so.3 /usr/${libdir}/lapack/openblas/liblapack.so
